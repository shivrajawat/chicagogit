...
if($_POST['OrdType']=='delivery')
{
	...
	{	
	 ...
	 if(!empty($isholiday))
		{
			echo "Today is holiday, Your order will be placed beginning the next time we are open";?>
			<input type="hidden" name="dhour" value="restorant_closed" />
		<?php }
		else
		{
			 ...
			 { 	
						...
						{ 
							$times = create_time_range($startEndTime['day_start_time'], $startEndTime['last_order_time'], '15 mins'); ?>
							 <select class="span12" name="dhour">
								...
								elseif($time>$lasttime)
								{
									if($eveningflag==1)
									{
										...
										//Restaurant is closed, is there are blank evening delivery hours
											echo "The store is closed for the day. Your order will be placed beginning the next time we are open."; 
								?>                                
											<input type="hidden" name="dhour" value="restorant_closed" />
								<?php }
								}
...
//										$eveningflag=1;
									...
									<?php /*?><select class="span12" name="hour">
						...
						<?php   }
								...
								{
									//If restaurant's evening delivery hours not started.
	//								if($morningflag==0)
		//							{
//										$iswholeday=1;
...
else 
{
 ...
 if(!empty($isholiday))
	{
		echo "Today is holiday, Your order will be placed beginning the next time we are open";?>
		<input type="hidden" name="hour" value="restorant_closed" />
	<?php }
	else
	{
		if($currentday==$getday)
