(function ($, undefined) {
	$(function () {
		...
		.delegate(".btnGetCoords", "click", function (e) {
			if (e && e.preventDefault) {
				e.preventDefault();
			}
			$.get("index.php?controller=pjAdminLocations&action=getCoords", {
				"address": $("#address").val()
			}).done(function (data) {
			...
			return false;
		}).delegate(".price-del", "click", function (e) {
			if (e && e.preventDefault) {
				e.preventDefault();
			}
			$(this).parent().parent().remove();
			return false;
		}).delegate(".btnAddPrice", "click", function (e) {
			if (e && e.preventDefault) {
				e.preventDefault();
			}
		...
		if ($frmCreateLocation.length > 0) {
			...
			$tabs.bind('tabsshow', function(event, ui) {
				if (ui.panel.id == 'tabs-2' && !myGoogleMaps) {
					myGoogleMaps = new GoogleMaps();
		  ...
		  google.maps.event.addDomListener(window, 'load', myGoogleMaps);
